...
hasGPU = True
print(os.path.join(ExopticonWorker.get_data_dir(), "libdark.so"))
lib = CDLL(os.path.join(ExopticonWorker.get_data_dir(), "libdark.so"), RTLD_GLOBAL)
...
def detect_image(net, meta, im, thresh=.5, hier_thresh=.5, nms=.45, debug= False):
    ...
    dets = get_network_boxes(net, custom_image.shape[1], custom_image.shape[0], thresh, hier_thresh, None, 0, pnum, letter_box) # OpenCV
...
class Yolo4Worker(ExopticonWorker):
    # Overloads
    def __init__(self):
