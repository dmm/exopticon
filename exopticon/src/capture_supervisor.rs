...
pub struct CaptureMetrics {
    ...
    pub frame_count: IntCounterVec,
}

#[derive(Clone)]
pub struct CaptureActorMetrics {
    pub frame_count: IntCounter,
}
...
impl CaptureMetrics {
    pub fn new() -> Self {
        Self {
            ...
            frame_count: IntCounterVec::new(
                opts!(
                    "capture_frame_count",
                    "Number of frames generated by capture worker"
                )
                .namespace("exopticon"),
                &["camera_id", "camera_name"],
            )
    ...
    pub fn get_actor_metrics(&self, actor_id: i32, actor_name: &str) -> CaptureActorMetrics {
        CaptureActorMetrics {
            frame_count: self
                .frame_count
    ...
    pub fn register(&self, registry: &Registry) -> Result<(), ()> {
        registry
            ...
            .map_err(|_| ())?;
        registry
            .register(Box::new(self.frame_count.clone()))
            .map_err(|_| ())?;
            ...
            critical_section::<Self, _>(async {
                with_ctx(|actor: &mut Self, ctx: &mut Context<Self>| {
                    for (storage_path, camera, addr) in &mut actor.workers {
                        let actor_metrics =
                            actor.metrics.get_actor_metrics(camera.id, &camera.name);
