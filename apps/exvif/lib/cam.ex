defmodule Exvif.Cam do
  ...
  def request_capabilities(url, username, password) do
    body = envelope_header(username, password)
    <>
    """
  ...
  def parse_profile(doc, token) do
    ...
    %{
      ...
      video_encoder_configuration:  %{
        ...
        rate_control: %{
          frame_rate_limit: to_int(sel.("/tt:VideoEncoderConfiguration/tt:RateControl/tt:FrameRateLimit")),
          encoding_interval: to_int(sel.("/tt:VideoEncoderConfiguration/tt:RateControl/tt:EncodingInterval")),
          bitrate_limit: to_int(sel.("/tt:VideoEncoderConfiguration/tt:RateControl/tt:BitrateLimit"))
        }
      }
    }
  end
  ...
  def fetch_camera(url, username, password) do
    ...
    uris = Enum.into(Enum.map(profiles, fn p -> { String.to_atom(p[:profile_token]), fetch_stream_uri(url, username, password, p[:profile_token]) } end),
      %{})
